...
| `ARCHIVE_API_KEY` | Bearer token required by `POST /api/archive/ingest`. Stored on Dean's Mac in macOS Keychain at service `LINKSBLUE_ARCHIVE_API_KEY` (matches the screaming-snake convention shared with `LINKSBLUE_WRITE_KEY`). Set on Railway as the env var `ARCHIVE_API_KEY`. |
...
**Where prompt and segue bodies live:** `TRIADBLUE/ai-archive/prompts/PROMPT_<MM-DD-YYYY-N>.md`, and response letters in the same flat directory as `PROMPT_<MM-DD-YYYY-N><letter>.md` (e.g. `PROMPT_05-05-2026-26a.md`). Mirrors `TRIADBLUE/ai-archive/segues/SEGUE_<MM-DD-YYYY-N>.md`. **Never** check prompt files into platform repos (`linksblue`, `consoleblue`, `swipesblue`, `businessblueprint`, `hostsblue`, `scansblue`, `builderblue2`, `tbsys`) — one canonical home keeps the bodies next to the ledger they reference.
...
**Agent code uses the MCP tools, not the REST endpoints.** The MCP server already holds `LINKSBLUE_WRITE_KEY` and applies it at call time, so an MCP-connected agent (Cowork, Claude.ai web, any future client) never needs the credential. REST is the surface for the local capture daemon, shell scripts, and humans (curl/Postman). If an agent reaches for `POST /api/archive/claim-number` directly, it's missing the MCP `claim_number` tool — fix the connection, don't smuggle the key. (Cowork's MCP client may need a reconnect after a Railway redeploy to pick up new tools.)
...
| 2026-05-04 | **Daemon — keychain service name renamed to `LINKSBLUE_ARCHIVE_API_KEY` (Prompt 05/04/2026-24).** Cosmetic but ecosystem-wide: align with the screaming-snake convention shared by `LINKSBLUE_WRITE_KEY` so all linksblue secrets on Dean's Mac use one naming style. Updated `daemon/daemon.js` `loadKey()` lookup, `daemon/install.sh` precheck, `daemon/test.sh`, `daemon/README.md`, root `README.md`, and root `CLAUDE.md` env-var table. Railway env var name (`ARCHIVE_API_KEY`) and the linksblue ingest-route bearer-check are unchanged — this is a Mac-side keychain rename only. Dean replaced the keychain entry first (`LINKSBLUE_ARCHIVE_API_KEY` added, dash-case `linksblue-archive-api-key` removed); this PR brings the daemon code up to match before the next daemon restart. Verified locally: `node -e` lookup against the new keychain entry returns the 64-char key. (Prompt 05/04/2026-24, Claude Code.) |
| 2026-05-05 | **Doc-only sync: `claim_number` / `claim_response` / `update_prompt_status` MCP tools were already shipped under v2.7 (Prompt 05/03/2026-13) and live on Railway, but the README's "Tools exposed" line still listed only the original 9 read tools — leading Cowork's gap analysis to assume the tools were missing and draft a duplicate-implementation prompt. (1) `README.md` "Tools exposed" rewritten as a versioned inventory of all 24 deployed tools with an explicit "agent code uses MCP, not REST" line. (2) `CLAUDE.md` adds an MCP-first rule paragraph immediately after the REST-mirror inventory: agents go through MCP (server holds `LINKSBLUE_WRITE_KEY` for them), REST stays for the daemon, scripts, and humans; if an agent reaches for `/api/archive/*` directly, that's a missing-tool signal — fix the connection, don't smuggle the key. Note about Cowork client reconnect after Railway redeploys included. (3) New canonical-home rule in `CLAUDE.md`: prompt bodies live at `TRIADBLUE/ai-archive/prompts/PROMPT_<MM-DD-YYYY-N>.md` (flat, no slug, mirroring `segues/`); responses share the same directory as letter-suffixed filenames; **never** in platform repos. The prompt body for this work was committed to ai-archive at `prompts/PROMPT_05-05-2026-26.md` (commit `a0291e7d`); a working draft was briefly checked in at `linksblue/prompts/drafts/...` then removed when Dean called out that the bootstrap-message template gave the wrong location. **No code changes** in `index.js`, `routes/`, `daemon/`, `package.json`, or any tool registration. Verified live MCP `tools/list` returns 24 tools including all three claim tools. The "code/auth/TTL" acceptance criteria from the prompt body were already satisfied by 05/03/2026-13; this commit closes only the docs gap. (Prompt 05/05/2026-26, Claude Code.) |
| 2026-05-09 | **Daemon JSONL parser — title selection skips `isMeta: true` lines (Prompt 05/09/2026-38).** When the daemon was reloaded on 2026-05-09 after a 3-day outage, the claude-code watcher surfaced 110 deltas of which 98 were rejected by the ingest endpoint with HTTP 409 "path collision". Diagnostic split the cause into two parts: dominantly, Claude Code sessions terminated with `/exit` consist entirely of harness-injected lines marked `isMeta: true` whose content (`<local-command-caveat>...`) is byte-for-byte identical across every such session — `parseJsonlFile` ignored the `isMeta` flag and used those lines' first 80 chars as the title, producing identical slugs and identical archive paths. Fix: title-selection branch in `daemon/lib/jsonl-parser.js` now reads `if (role === 'user' && !titleCandidate && entry.isMeta !== true)` (added third clause). Comment block above the branch explains where `isMeta` comes from, why it's skipped, the silent-regression risk if Anthropic renames or removes the flag, and the explicit note that meta lines remain in `messages[]` (excluded only from title selection, not from the captured transcript). New unit-test suite at `daemon/lib/__tests__/jsonl-parser.test.js` using Node 22's built-in `node:test` (zero new dependencies); 4 cases — meta-first title fallthrough, all-meta produces null titleCandidate, no-meta-flags regression guard, meta-line preservation in `messages[]`. `daemon/package.json` `test` script now runs unit tests (`node --test lib/__tests__/*.test.js`); the existing interactive round-trip script preserved as `test:integration` (`./test.sh`). No change to `watchers/claude-code.js`, `watchers/cowork.js`, `watchers/claude-leveldb.js`, `daemon.js`, or any backend route — the watcher-level fallback `Claude Code session ${sourceId.slice(0, 8)}` already handles the case where `parseJsonlFile` returns a null `titleCandidate`. Endpoint-side path-discriminator follow-up is Prompt 05/09/2026-39. (Prompt 05/09/2026-38, Claude Code.) |
| 2026-05-09 | **Archive-ingest endpoint — source_id-derived path discriminator (Prompt 05/09/2026-39).** Layer 2 of the 2026-05-09 409-bleed fix. Survey of real Claude Code JSONL during Prompt 38's verification revealed only `<local-command-caveat>` lines carry `isMeta: true`; `<command-name>/exit</command-name>` and `<local-command-stdout>` lines do not. So the watcher fix from 38 moves the title from one piece of harness boilerplate to another, both deterministic across sessions — slug-collision pattern persists at a different slug. This prompt eliminates the entire collision failure mode at the path layer. (1) `routes/archive-ingest.js` now imports `crypto`. (2) New helper `pathDiscriminator(sourceId)` — first 8 hex chars of `sha256(sourceId)`. ~4.3B values; birthday-collision probability ≈ 1-in-100 across 1M files. (3) `pathFor()` signature gains a required `sourceId` parameter; output filename becomes `{date}-{platform}-{slug}-{disc8}.md`. (4) Both call sites (`handleModeA`, `handleModeB` first-snapshot branch) pass `sourceId: source_id`. (5) Comment block above `pathDiscriminator` documents the design, the legacy-file backward-compat guarantee (legacy files at pre-discriminator paths are NOT migrated; `ghSearchSourceId` finds them via frontmatter content not path; Mode B append continues writing to `existingPath`), and the discriminator-collision risk. (6) Defense-in-depth 409 fallback in `handleModeA` preserved — now expected to be unreachable; if it ever fires post-fix, that's a sha256 collision worth investigating. (7) New unit-test suite at `routes/__tests__/archive-ingest.test.js` using `node:test` (zero new dependencies); 5 cases — discriminator format (`/^[0-9a-f]{8}$/`), determinism, distinct source_ids produce distinct paths, idempotency, UTC date encoding. (8) Root `package.json` gains a `test` script (`node --test routes/__tests__/*.test.js`). No change to `daemon/`, `routes/archive-helpers.js`, `routes/archive-last-message-count.js`, `index.js`, or any MCP tool registration. Endpoint becomes structurally collision-proof up to a sha256 collision. (Prompt 05/09/2026-39, Claude Code.) |
| 2026-05-09 | **Daemon — end-to-end health monitoring (Prompt 05/09/2026-44).** Three pieces, single contract, single PR — closes operational finding (2) of segue 41 (3-day silent outage went undetected). (1) Daemon side: new `postHeartbeat(passInfo, key)` helper in `daemon/daemon.js` POSTs `{daemon_id, host, timestamp, last_pass_status, watchers, queue_depth}` to the new endpoint after each successful `runSnapshot()`. `runSnapshot` now tracks per-watcher `{deltas, ok}` and a pass-level `last_pass_status` of `ok`/`degraded`/`failed`; data lives in a local `passInfo` object (does NOT mutate `state` so it stays out of state.json). Heartbeat failure is logged-and-swallowed — monitoring can never break primary capture. Bearer auth uses the existing `LINKSBLUE_ARCHIVE_API_KEY` keychain entry; no new secret. (2) Linksblue side: new `routes/daemon-heartbeat.js` exporting `POST /heartbeat` (bearer-authed via existing `requireBearer` from archive-helpers, validates schema, persists to `TRIADBLUE/ai-archive` at `monitoring/heartbeat-{daemon_id}.json` for cross-redeploy durability + audit-via-git-log) and `GET /heartbeat` (no auth, in-memory cache + cold-start ai-archive fallback, returns payload + derived `seconds_since`). One mount line added to `index.js` between the existing `archive-last-message-count` mount and the health-check handler — no other index.js changes. Persistence is fail-soft (in-memory cache absorbs GH outages). (3) Action side: new `.github/workflows/daemon-health-check.yml` with `cron: */15 * * * *` + `workflow_dispatch`, uses `actions/github-script@v7` to GET the endpoint, opens a `daemon-health`-labeled issue if heartbeat is ≥45min stale OR `last_pass_status !== "ok"` OR endpoint returns 404/error/unreachable. Title-based dedupe — first stale check opens an issue, subsequent checks comment, healthy check closes. Worst-case detection lag: 15 (Action interval) + 45 (staleness threshold) = 60 minutes vs the 3-day silent outage that motivated the work. (4) Tests: new `routes/__tests__/daemon-heartbeat.test.js` (13 cases — pure-fn validation/seconds-since coverage + integration-via-in-process-Express coverage of all five AC HTTP behaviors; zero new deps via `node:http` + injectable persistence stub). 18/18 root tests pass; 8/8 daemon tests pass. (5) Docs: `daemon/README.md` gains a "Health monitoring" section + heartbeat step in the snapshot loop. The contract is indivisible (daemon writes, endpoint receives, Action reads — any one alone is dead code), justifying single-PR bundling per the prompt's CONTEXT note despite Rule 10. No changes to `daemon/lib/`, `watchers/*`, archive-ingest, archive-helpers, archive-last-message-count, MCP tools, or any `/api/github/*` or `/api/archive/*` route. (Prompt 05/09/2026-44, Claude Code.) |
